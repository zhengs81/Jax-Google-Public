...
def main(_):
  ...
  for epoch in range(1, FLAGS.epochs + 1):
    ...
    for _ in range(num_batches):
      if FLAGS.dpsgd:
        # pr.enable()
        # jax.profiler.start_trace("/tmp/private")
        opt_state = \
            private_update(
        ...
        # pr.disable()
        # pr.dump_stats("private.prof")
      else:
        # pr.enable()
        # jax.profiler.start_trace("/tmp/public")
        opt_state = update(
            key, next(itercount), opt_state, shape_as_image(*next(batches)))
        # opt_state.block_until_ready()
        # jax.profiler.stop_trace()
        # pr.disable()
        # pr.dump_stats("public.prof")
    pr.disable()
    # pr.dump_stats("private_new.prof")
    # pr.dump_stats("public_new.prof")
